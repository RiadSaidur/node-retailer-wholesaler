...
POST http://localhost:5000/register?type=wholeseller HTTP/1.1
...
GET http://localhost:5000/auth/user?email=salesman@example.com HTTP/1.1
```
...
```json
// required query param: ?email=
DELETE http://localhost:5000/user?email=salesman@example.com HTTP/1.1
```
...
GET http://localhost:5000/user/all?sort=-1&page=0 HTTP/1.1
```
